...
class EventDetailsViewModel with ChangeNotifier {
  ...
  bool get canAddMoreTickets => !isEventExpired && _ticketCount < ticketsRemaining;
  ...
  void decreaseTicketCount() {
    if (!isEventExpired && _ticketCount > 1) {
      _ticketCount--;
      notifyListeners();
    }
  }
  ...
  Future<void> bookEvent() async {
    if (isEventExpired) throw Exception('This event has already ended');
