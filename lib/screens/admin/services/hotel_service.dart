...
class HotelService {
  ...
  Future<void> addHotel(Map<String, dynamic> hotelData) async {
    ...
    // Extract existing hotel IDs like hotel01, hotel02, ...
    final existingIds = snapshot.docs
        .map((doc) => int.tryParse(doc.id.replaceAll('hotel', '')))
        .whereType<int>()
        .toList();
  ...
  Future<List<HotelModel>> fetchHotels() async {
    ...
    return snapshot.docs
      ...
      try {
        ...
        .whereType<HotelModel>() // Filters out any nulls from failed parsing
        .toList();
  }

  // Optional: Fetch only hotels managed by a specific manager
